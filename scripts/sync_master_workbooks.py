...
SPECIAL_AI: dict[str, str] = {
    ...
    "MAC0001": "KI: Ich verstehe MAC0001 als temporaeren IBN-/Testbefehl von Microtom an den Raspi. Der Raspi uebersetzt diesen Wert in produktionsnahe Startkommandos fuer ESP32-PLC und Wickler. Dieser Parameter ist ausdruecklich nur fuer Inbetriebnahme/Test vorgesehen und muss spaeter entfernt oder durch produktive MAS/MAP-Ablaufbefehle ersetzt werden. Der ESP soll ihn gemaess Masterliste nicht direkt als normalen Parameter verwenden.",
    "MAC0002": "KI: Ich verstehe MAC0002 als temporaere Labellaenge bzw. Taktstrecke fuer den IBN-Test. Default 1000 bedeutet 100.0 mm. Spaeter kommt dieser Wert aus Rezept-/Formatdaten und Etiketteneinmessung, nicht mehr aus einem MAC-Testparameter.",
    "MAC0003": "KI: Ich verstehe MAC0003 als temporaere Vorzugsgeschwindigkeit fuer Motor 3 im IBN-Test. Wird kein neuer Wert gesetzt, verwendet der Raspi den zuletzt gespeicherten Wert. Produktiv wird die Geschwindigkeit ueber MAP0014 bzw. die Betriebsart vorgegeben.",
    "MAC0004": "KI: Ich verstehe MAC0004 als temporaere Rampe fuer Beschleunigung und Bremsung des Etikettenantriebs im IBN-Test. Produktiv wird diese Vorgabe aus Maschinen- bzw. Formatparametern abgeleitet und darf den Etikettenschlupf nicht erhoehen.",
    "MAC0005": "KI: Ich verstehe MAC0005 als temporaere Wegvorgabe fuer kontinuierliches Vorziehen oder Rueckspulen. Default 0 bedeutet endlos fahren bis MAC0001=0. Produktiv wird diese Laenge aus Ablaufzustand, Rueckspulweg oder Bedienhandlung bestimmt.",
    "MAC0006": "KI: Ich verstehe MAC0006 als temporaere Zyklusanzahl fuer den getakteten IBN-Test. Default 20 entspricht einem kurzen Stabilitaetstest; 0 bedeutet laufender Taktbetrieb bis MAC0001=0. Spaeter wird diese Logik durch den echten Produktionsablauf und das Schieberegister ersetzt.",
