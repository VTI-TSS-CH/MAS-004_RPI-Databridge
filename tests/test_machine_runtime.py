...
class MachineRuntimeTests(unittest.TestCase):
    ...
    def test_entering_stop_sends_defined_axis_positions_once(self):
        ...
        with self.patched_runtime_external_io(runtime), patch("mas004_rpi_databridge.machine_runtime.EspMotorClient") as client_cls:
            ...
            def fake_refresh(motor_id):
                targets = {2: 200, 5: 0, 6: -200, 7: -200, 8: 1000, 9: 1000}
                return {
                    "motor": {
                        "state": {
    ...
    def test_setup_resumes_from_motion_recovery_event_when_state_field_was_overwritten(self):
        runtime = self.build_runtime()
        runtime._write_state(
            current_state=9,
            requested_state=9,
        ...
        info = dict(waiting["info"])
        self.assertTrue(info["setup"]["motion_recovery_pending"])
        # Simulate the real race seen on the machine: the recovery completion
        # survived in machine_events, but a parallel refresh overwrote
        # info.safety.last_motion_recovery.
        ...
        runtime._record_event(
            "reset_motion_recovery",
            "info",
            "Safety-Reset Motion-Recovery abgeschlossen",
            {"ok": True, "finished_ts": now_ts()},
        )
    ...
    def test_stale_setup_motion_recovery_pending_is_dropped_without_motion(self):
        runtime = self.build_runtime()
        stale_ts = now_ts() - (machine_runtime_module._SETUP_MOTION_RECOVERY_PENDING_MAX_AGE_S + 5.0)
        runtime._write_state(
            current_state=3,
            requested_state=3,
            state_source="test",
            ...
            info={
                "setup": {
                    "mas0002_setup_seen_ts": stale_ts,
                    "motion_recovery_pending": True,
                    "motion_recovery_pending_command_ts": stale_ts,
                    "motion_recovery_pending_ts": stale_ts,
                    "last_result": {"waiting": True, "reason": "reset_motion_recovery_in_progress"},
                }
            },
        )
        ...
        controller_cls.return_value.run.assert_not_called()
        setup = snapshot["info"]["setup"]
        self.assertNotIn("motion_recovery_pending", setup)
        self.assertEqual("stale_motion_recovery_pending", setup["last_result"]["reason"])
        self.assertEqual(9, snapshot["requested_state"])
    ...
    def test_stop_axis_positions_are_not_ok_when_drive_accepts_but_does_not_move(self):
        ...
        with self.patched_runtime_external_io(runtime), patch("mas004_rpi_databridge.machine_runtime.EspMotorClient") as client_cls:
            ...
            client.refresh.side_effect = lambda motor_id: {
                "motor": {
                    "state": {
                        "feedback_tenths_mm": {2: 200, 6: 300}.get(int(motor_id), 1000),
                        "move": False,
                        "busy": False,
                        "alarm": False,
                        "hwto": False,
    ...
    def test_existing_stop_state_revalidates_old_stop_position_result(self):
        ...
        with self.patched_runtime_external_io(runtime), patch("mas004_rpi_databridge.machine_runtime.EspMotorClient") as client_cls:
            ...
            client.refresh.side_effect = lambda motor_id: {
                "motor": {
                    "state": {
                        "feedback_tenths_mm": {2: 200, 5: 0, 6: -200, 7: -200, 8: 1000, 9: 1000}[int(motor_id)],
                        "move": False,
                        "busy": False,
                        "alarm": False,
                        "hwto": False,
        ...
        self.assertEqual(True, snapshot["info"]["stop_positions"]["ok"])
        self.assertEqual(12, snapshot["info"]["stop_positions"]["logic_version"])
    ...
    def test_stop_axis_position_commands_inside_active_min_limit(self):
        ...
        ok, msg = self.params.set_value("MAS0002", "2", actor="microtom")
        self.assertTrue(ok, msg)
        positions = {2: 200, 5: 380, 6: -200, 7: -200, 8: 1000, 9: 1000}
        ...
        with self.patched_runtime_external_io(runtime), patch("mas004_rpi_databridge.machine_runtime.EspMotorClient") as client_cls:
            ...
            def fake_refresh(motor_id):
                ...
                return {
                    "motor": {
                        "state": {
                            "feedback_tenths_mm": -6508 if motor_id == 7 else {2: 200, 5: 0, 6: -200, 8: 1000, 9: 1000}.get(motor_id, 0),
                            "move": False,
                            "busy": False,
                            "alarm": motor_id == 7,
    ...
    def test_safety_reset_pulses_then_stays_latched_when_estop_remains_active(self):
        runtime = self.build_runtime()
        self.params.apply_device_value("MAS0001", "21", promote_default=True)
        self.params.apply_device_value("MAS0028", "1", promote_default=True)
        ok, msg = self.params.set_value("MAS0002", "2", actor="microtom")
        ...
        with (
            ...
            patch.object(runtime, "_pulse_esp_reset_output", return_value={"ok": True}) as pulse,
            patch.object(runtime, "_wait_for_esp_command_endpoint", return_value={"ok": True}) as wait_endpoint,
        ):
            snapshot = runtime.refresh()

        refresh_single.assert_called()
        pulse.assert_called_once()
        wait_endpoint.assert_called_once()
        self.assertEqual(21, snapshot["current_state"])
    ...
    def test_safety_reset_releases_after_estop_recovers_from_reset_pulse(self):
        runtime = self.build_runtime()
        self.params.apply_device_value("MAS0001", "21", promote_default=True)
        self.params.apply_device_value("MAS0028", "1", promote_default=True)
        ok, msg = self.params.set_value("MAS0002", "2", actor="microtom")
        ...
        def refresh_after_reset(device_code, pin_labels=None):
            if "I0.7" in {str(pin) for pin in (pin_labels or set())}:
                self.io_store.upsert_value("esp32_plc58__I0_7", "1", "live", "test-reset")
            return {"ok": True}

        with (
            patch.object(runtime, "_refresh_single_io_device", side_effect=refresh_after_reset),
            patch.object(runtime, "_pulse_esp_reset_output", return_value={"ok": True}) as pulse,
            patch.object(runtime, "_wait_for_esp_command_endpoint", return_value={"ok": True}),
            patch.object(runtime, "_reset_esp_process_runtime", return_value={"ok": True}),
            patch.object(runtime, "_start_reset_motion_recovery_background", return_value={"ok": True, "queued": True}),
        ):
            snapshot = runtime.refresh()

        pulse.assert_called_once()
        self.assertEqual(9, snapshot["current_state"])
        self.assertEqual("safety_reset_ready", runtime._state_row()["state_source"])
        self.assertFalse(snapshot["purge_active"])
        self.assertEqual("0", self.params.get_effective_value("MAS0002"))
        self.assertEqual("0", self.params.get_effective_value("MAS0028"))
        safety = snapshot["info"]["safety"]
        self.assertFalse(safety["latched"])
        self.assertEqual("ready", safety["phase"])
    ...
    def test_label_removal_required_while_paused_updates_without_rewind(self):
        runtime = self.build_runtime()
        runtime._write_state(
            current_state=13,
            requested_state=13,
        ...
        runtime._execute_label_removal_rewind = Mock(
            return_value={"ok": True, "label_no": 3, "rewind_executed": True}
        )
        runtime._sync_esp_machine_state = Mock(return_value=True)
        ...
        production_info = snapshot["info"][PRODUCTION_RUNTIME_INFO_KEY]
        self.assertFalse(production_info["label_removal_request"]["rewind_required"])
        self.assertFalse(production_info["label_removal_request"]["rewind_executed"])
        self.assertEqual("already_in_label_removal_pause", production_info["label_removal_request"]["rewind"]["skipped"])

    def test_expected_removed_but_seen_while_paused_starts_label_removal_rewind(self):
        runtime = self.build_runtime()
        runtime._write_state(
            current_state=13,
            requested_state=13,
        ...
        runtime._execute_label_removal_rewind = Mock(
            return_value={"ok": True, "label_no": 3, "distance_mm": 104.5, "rewind_executed": True}
        )
        runtime._sync_esp_machine_state = Mock(return_value=True)
        ...
        result = runtime.handle_event(
            {
                ...
                "rewind_mode": "tact_after_current_tact",
                "rewind_distance_mm": 104.5,
                "rewind_after_label_no": 12,
            }
        )

        self.assertTrue(result["ok"])
        runtime._execute_label_removal_rewind.assert_called_once_with(label_no=3, distance_mm=104.5)
        snapshot = runtime.snapshot()
        production_info = snapshot["info"][PRODUCTION_RUNTIME_INFO_KEY]
        self.assertTrue(production_info["label_removal_request"]["rewind_required"])
        self.assertTrue(production_info["label_removal_request"]["rewind_executed"])
    ...
    def test_production_esp_monitor_fallback_skips_first_wickler_when_ready_already_seen(self):
        self.cfg.esp_simulation = False
        runtime = self.build_runtime()
        self.mark_production_active(runtime)
