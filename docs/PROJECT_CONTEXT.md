...
- MOXA access is serialized per `host:port`, unchanged output writes are suppressed, and status-lamp writes are best-effort with a short error cooldown. This keeps transient ioLogik timeouts from blocking the machine runtime or indirectly disturbing ESP motor communication.
