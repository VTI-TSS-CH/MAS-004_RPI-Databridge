...
- For Smart Wickler integration:
  ...
  - The measuring run expects ESP firmware support for `MOTOR 3 MOVE_REL_MM=...` on the Direct-Data path. Do not use `MOVE_REL_MM_OP` for this workflow on the production machine: it can acknowledge the command after AZD I/O reconfiguration without causing physical motion.
